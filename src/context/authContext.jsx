...
export const AuthProvider = ({ children }) => {
  ...
  const [binaryConverted, setBinaryConverted] = useState(null);
  ...
  const convertNumber = async (from, number) => {
    try {
      if (number) {
        if (from == "decimal") {
          const res = await convertFromDecimal(number);
          if (res.data.error == 0) {
            ToastCustom("success", res.data.message, "Success", "top-right");
            setBinaryConverted(res.data);
          } else {
            setBinaryConverted(null);
            ToastCustom("error", res.data.message, "Error", "top-right");
          }
        }
        if (from == "binary") {
          const res = await convertFromBinary(number);
          if (res.data.error == 0) {
            ToastCustom("success", res.data.message, "Success", "top-right");
            setBinaryConverted(res.data);
          } else {
        ...
        if (from == "octal") {
          const res = await convertFromOctal(number);
          if (res.data.error == 0) {
            ToastCustom("success", res.data.message, "Success", "top-right");
            setBinaryConverted(res.data);
          } else {
        ...
        if (from == "hexadecimal") {
          const res = await convertFromHexadecimal(number);
          if (res.data.error == 0) {
            ToastCustom("success", res.data.message, "Success", "top-right");
            setBinaryConverted(res.data);
          } else {
        ...
        ToastCustom(
          "error",
          "Field number is empty, enter a number please!",
          "Error Message",
          "top-right"
        );
      }
